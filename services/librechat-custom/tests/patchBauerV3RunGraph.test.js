...
test('run-graph patch is checksum-bound and propagates only the private V3 marker', () => {
  ...
  assert.match(patched, /bauerV3DirectFinal\?: boolean/);
  assert.match(patched, /\{ toolEnd: true \}/);
  assert.doesNotMatch(patched, /agents\[0\]\.toolEnd/);
});

test('run-graph patch refuses absent or ambiguous anchors', () => {
  assert.throws(
